...
Pro-button → waitlist modal), `how-it-works.html` (methodology page),
`enterprise.html` (the club-facing scouting-department demo, below), and
`worldcup.html` (World Cup 2026 editorial special: eight breakout stars,
attributed media rumors vs engine-computed best-fit clubs — generated by
`code/worldcup.ts`, which runs the real engine at build time over authored
Tier-1 profiles; research snapshot July 19, 2026, sources cited on-page).

### Enterprise demo (`site/enterprise.html`)

A simulated club workspace showing the enterprise product. It boots into
**Northbridge United** — a fictional "mature PitchIntel club" (defined in
`code/northbridge.ts`) with an 18-man squad carrying ALL three data tiers
(~112 populated dims each, 100% profile confidence), a Today morning-briefing
view (overnight reports, contract alerts, board queries, decision log), a
Squad view (market vs internal valuation, succession/extension flags), and a
shortlist deep in multi-report state including one scouted-and-killed target.
"Day in the life of a mature PitchIntel club."

Switching club (sidebar) gives the lighter generic workspace for any of the
30 real clubs: Discovery from public Tier 1 data, scout assignments, Tier 3
report filing with live valuation impact, DoF dashboard. State persists in
localStorage; "Reset demo" starts over.

The World Cup "Crazy Files" players (`code/data/wc-files.ts`, shared with the
World Cup page) are injected into the demo's scoutable pool with a 🏆 badge
and pre-seeded onto the Northbridge shortlist with authored scout reports.
